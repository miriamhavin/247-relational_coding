...
def plot_results(df, pdf, mode):
    ...
    ax.set(xlabel='Seconds', ylabel='Frequency', title=f'All Datum ({len(word_len)} words)')
    pdf.savefig(fig)
    plt.close()
...
def plot_results_convo(df, pdf, mode, convo_id):
    ...
    fig, ax = plt.subplots(figsize=(15,6))
    ax.hist(word_len, log=True, bins=100)
    ax.set(xlabel='Seconds', ylabel='Frequency', title=f'Conversation {convo_id} ({len(word_len)} words)')
    pdf.savefig(fig)
    plt.close()
...
def plot_results_all_sid(sid_list, pdf, mode):
    ...
    for sid in sid_list:
        ds = load_pickle(os.path.join('data/tfs',str(sid),'pickles',str(sid)+'_full_labels.pkl'))
        df = pd.DataFrame(ds['labels'])
        word_len = get_results(df, mode)

        fig, ax = plt.subplots(figsize=(15,6))
        ax.hist(word_len, log=True, density=True, bins=100)
        ax_all.hist(word_len, log=True, density=True, bins=100, alpha=0.5, label=sid)
        ax.set(xlabel='Seconds', ylabel='Density', title=f'{sid} All Datum ({len(word_len)} words)')
        pdf.savefig(fig)
