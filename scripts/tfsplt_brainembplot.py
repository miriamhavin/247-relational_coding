...
from statsmodels.stats import multitest, stattools
...
dirs = [
    "0shot-zz-podcast-full-777-gpt2-xl-polydet2-nnt/777/",
...
dirs = [
    "0shot-kw-podcast-full-777-gpt2-xl-polydet2-nnt/777/",
...
def run_exp(experiment, elecs, y_nn_idxs):
    ...
    for i, resultdir in enumerate(dirs):
        ...
        if i == 0:  # not the shuffled one
            ...
            ax.fill_between(
                xaxis, mean_nn - err_nn, mean_nn + err_nn, alpha=0.1, color="red"
            )
            ...
            ax.fill_between(
                xaxis, mean_nnt - err_nnt, mean_nnt + err_nnt, alpha=0.1, color="orange"
            )
        ...
        if i == 0 and nperms > 0:
            ...
            yheight = ax.get_ylim()[1] - 0.005
            ax.scatter(
                lags[siglags], [yheight] * len(siglags), marker="*", color="green"
            )
            ...
            # sig test for nnt
            pvals = [
                paired_permutation(corrs4[i], corrs[i], nperms)
                for i in range(len(lags))
            ]
            pcorr = fdr(pvals)
            issig = (m > minP) & (pcorr < 0.01)
            ...
            yheight = ax.get_ylim()[1] - 0.005
            ax.scatter(
                lags[siglags], [yheight] * len(siglags), marker="*", color="orange"
            )
...
def get_zeroshot_datum(df, is_gpt2, kfolds=10):
    ...
    for i in range(kfolds):
        ...
        ret_folds_idxs.append(
            (test_index, test_nn_index, train_index, test_test_nn_index)
        )
